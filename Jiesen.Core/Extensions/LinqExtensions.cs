...
namespace Jiesen.Core.Extensions
{
    public static class LinqExtensions
    {
        ...
        public static string FieldName<TEntity>(int value)
            where TEntity : new()
        {
            var entity = Activator.CreateInstance<TEntity>();
            var property = typeof(TEntity).GetFields().ToList().Find(p =>
            {
                var i = (int) p.GetValue(entity) == value;
                return i;
            });
