    ...
    public static class AppConfig
    {
        ...
        public static string GetAbsolutePath(string dllConfigName)
        {
            ...
            if (string.IsNullOrEmpty(enviromentParam))
                throw new FileNotFoundException(dllConfigName + "未找到;" + "EnvironmentParam未配置");
        ...
        public static string GetFileByAbsolutePath(string fileName)
        {
            ...
            var enviromentParam = AppSettings["EnvironmentParam"];
            if (string.IsNullOrEmpty(enviromentParam))
                throw new FileNotFoundException(fileName + "未找到;" + "EnvironmentParam未配置");
        ...
        {
            var path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Configs", $"{dllConfigName}.config");
            if (File.Exists(path))
                return true;
            var enviromentParam = AppSettings["EnvironmentParam"];
            ...
            path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Configs", enviromentParam,
                $"{dllConfigName}.config");
